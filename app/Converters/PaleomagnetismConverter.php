    ...
    public function ExcelToJson($filepath)
    {
        ...
        foreach ($data as $rootNode) {
            switch ($rootNode['value']) {
                case 'Apparatus':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'E', $spreadsheet->getSheetByName('Apparatus'));
                    // $rootNode = $this->definitionForRoot($rootNode, 'C', 'definition-link', $spreadsheet->getSheetByName('Apparatus'));
                    $newData[] = $rootNode;
                    break;
                case 'Environment control':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'D', $spreadsheet->getSheetByName('Environment control'));
                ...
                case 'Inferred behavior':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'D', $spreadsheet->getSheetByName('Inferred behavior'));
