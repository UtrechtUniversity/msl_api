    ...
    public function ExcelToJson($filepath)
    {
        ...
        foreach ($data as $rootNode) {
            switch ($rootNode['value']) {
                case 'Apparatus':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'C', $spreadsheet->getSheetByName('Apparatus'));
                    $newData[] = $rootNode;
                    break;
                case 'Ancillary equipment':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'C', $spreadsheet->getSheetByName('Ancillary equipment'));
                    $newData[] = $rootNode;
                    break;
                case 'Technique':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'E', $spreadsheet->getSheetByName('Technique'));
                    $newData[] = $rootNode;
                    break;
                case 'Analyzed feature':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'E', $spreadsheet->getSheetByName('Analyzed feature'));
                    $newData[] = $rootNode;
                    break;
                case 'Inferred parameter':
                    $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'D', $spreadsheet->getSheetByName('inferred parameter '));
                    $newData[] = $rootNode;
                    break;
                default:
                    break;
