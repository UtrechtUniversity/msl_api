...
class SubsurfaceConverter
{
    public function ExcelToJson($filepath)
    {
        ...
        foreach ($nestedNodes as $rootNode) {
            $rootNode['subTerms'] = $this->checkRootNode($rootNode, 'D', $spreadsheet->getSheetByName('(sub)surface utlization setting'));
            $rootNode = $this->definitionForRoot($rootNode, 'D', $spreadsheet->getSheetByName('(sub)surface utlization setting'));
            $newData[] = $rootNode;
        }
        $nestedNodes = $newData;
