...
const CLOSE_ICON = `
        <svg class="close-filter-icon" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path
...
function createKeywordElement({
    ...
    const htmlString = `  
                <div id=${id} class=" applied-filter-card">
                    <div class="word-card truncate">
                            ${CLOSE_ICON}
                        <text class="word-value text-item"> ${displayName.substring(0, 40)} </text>
                    </div>
