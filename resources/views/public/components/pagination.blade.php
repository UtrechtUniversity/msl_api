<div class="self-center join p-4 rounded-xl divide-primary-100 divide-x-1">
    ...
    <a href="{{ $paginator->previousPageUrl() }}">
        <button class="pagination-button pagination-button-last-left"> <x-ri-arrow-left-s-line class="chevron-icon" />
        </button>
    </a>
        ...
        @for ($i = 1; $i < $count + 1; $i++)
            @if ($i == $currentPage)
                <a href="{{ $paginator->url($i) }}">
                    <button class="pagination-button pagination-button-active-page">{{ $i }}</button>
                </a>
            @else
                <a href="{{ $paginator->url($i) }}">
    ...
    @else
        @if (1 == $currentPage)
            <a href="{{ $paginator->url(1) }}">
                <button class="pagination-button pagination-button-active-page">{{ 1 }}</button>
            </a>
        @else
            <a href="{{ $paginator->url(1) }}">
        ...
        @if ($count == $currentPage)
            <a href="{{ $paginator->url($count) }}">
                <button class="pagination-button pagination-button-active-page">{{ $count }}</button>
            </a>
        @else
            <a href="{{ $paginator->url($count) }}">
                <button class="pagination-button">{{ $count }}</button>
            </a>
        @endif
    ...
    <a href="{{ $paginator->nextPageUrl() }}">
        <button class="pagination-button pagination-button-last-right"> <x-ri-arrow-right-s-line class="chevron-icon" />
        </button>
    </a>
