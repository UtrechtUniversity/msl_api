                                ...
                                markers.on('clusterclick', function(a) {
                                    ...
                                    for (childmarker of childMarkers) {
                                        if (first) {
                                            ...
                                            if (i > 0) {
                                                childContent = childContent + '<a onclick="prevPage(' + childMarkers[i - 1]
                                                    ._leaflet_id +
                                                    ')">prev</a>';
                                            }
                                            if (i < (childMarkers.length - 1)) {
                                                childContent = childContent + '<a onclick="nextPage(' + childMarkers[i + 1]
                                                    ._leaflet_id +
                                                    ')">next</a>';
                                            }
                                    ...
                                    if (elem) {
                                        ...
                                        if (markers.getLayer(leaflet_id).feature.properties.area_geojson) {
                                            if (markers.getLayer(leaflet_id).feature.properties.area_geojson.length > 0) {
                                                extraPopupLayer = L.layerGroup();
                                                L.geoJSON(JSON.parse(markers.getLayer(leaflet_id).feature.properties.area_geojson))
                                                    .addTo(
                                                        extraPopupLayer);
                                                map.addLayer(extraPopupLayer);
                                            }
